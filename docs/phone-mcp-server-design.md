...
首页采用底部“文件 / MCP / 插件 / 任务 / 日志”导航，“MCP”为第二个标签页。MCP 页面
顶部提供“设置 / 列表”二级标签：“设置”展示手机 MCP 服务配置，“列表”直接读取服务端
注册定义，按只读和控制能力分组展示全部工具、参数及安全标记；“设置 → 手机 MCP 服务”
仍保留为辅助入口。底部导航适配 Android
